...
import { customerSupportUrl, customersUrl, mealPlanUrl } from '../../assets/Api/api'
...
export default function CustomerReport() {
    ...
    const columns = [
        ...
        {
            name: 'Closing date',
            selector: row => row.closedAt ? row.closedAt.split('T')[0] : "-",
            sortable: true,
            compact: true
        },
        {
        ...
        {
            name: 'Status',
            selector: row => checkSupportStatus(row.status),
            sortable: true,
            compact: true
        }
    ]
    ...
    const getData = (values) => {
        ...
        axios.post(`${customerSupportUrl}/GetCustomerSupportsForReportOnFilter`, data)
            .then(response => {
                console.log(response);
                setReports(response)
                setCustomerSupports(response.data.data.customerSupport)
            })
            .catch(error => {
    ...
    const checkSupportStatus = (status) => {
        var result;
        switch (status) {
            case 1:
            case "1":
    ...
    return (
        <AccountLayout title="Customer support report">
            <BodyContainer>
                                        ...
                                        <div class="mb-3 col-md-12 my-0">
                                            <div class="row">
                                                ...
                                                <div class="col-sm-4">
                                                    ...
                                                    <input class="form-control" id="txtSearchCustomerName" type="text" name='ticketNumber' value={postData.ticketNumber} onChange={handleSearch} />
                                                </div>
                                                <div class="col-sm-2">
