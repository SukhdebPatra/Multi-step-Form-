...
import { customersUrl, vendorCouponsUrl } from '../../assets/Api/api'
...
export default function CustomerReport() {
    ...
    const columns = [
        ...
        {
            name: 'Total discount',
            selector: row => (row.symbol + row.discounts),
            sortable: true,
            compact: true
        },
        {
    ...
    const [couponsReport, setCouponsReport] = useState([])
    const [postData, setPostData] = useState(rdata)

    const getData = (values) => {
        let data = values
        data.status = values.status == 1 ? true : false
        axios.post(`${vendorCouponsUrl}/GetCouponsForReport`, data)
            .then(response => {
                console.log(response);
                setReports(response)
                setCouponsReport(response.data.data.couponOrders)
            })
            .catch(error => {
                                        ...
                                        <div class="mb-3 col-md-12 my-0">
                                            <div class="row">
                                                ...
                                                <div class="col-sm-4">
                                                    ...
                                                    <input class="form-control" id="txtSearchCustomerName" type="text" name='vendorCouponsId' value={postData.vendorCouponsId} onChange={handleSearch} />
                                                </div>
                                                <div class="col-sm-2">
